...
// Create a color scale mapping project names to colors
const colorScale = d3
  .scaleOrdinal<string, string>()
  .domain(sampleData.map((project) => project.name))
  .range(colorPalette);
...
const App: React.FC = () => {
  ...
  const updateVisibleData = (timeIndex: number) => {
    ...
    setTopProjects(new Set(topFiveIds));
              ...
              legends={[
                {
                  ...
                  data: Array.from(topProjects).map((id) => ({
                    id,
                    label: id,
                    color: colorScale(id),
                  })),
 ...
 * Displays event lines with floating titles, descriptions, and icons.
